...
export default function WeeklyProfileHeatmap({ profile, selectedWeekdays, isMobile, pdfMode, chartRef, exportActions }: Props) {
  ...
  const displaySlots = useMemo(() => {
    ...
    sourceSlots.forEach((slot) => {
      const startMinute = rangeStart + Math.floor((slot.startMinute - rangeStart) / displaySlotMinutes) * displaySlotMinutes;
  ...
  const rowStarts = useMemo(() => {
    const result: number[] = [];
    for (let minute = rangeStart; minute < rangeEnd; minute += displaySlotMinutes) result.push(minute);
    return result;
  }, [displaySlotMinutes, rangeEnd, rangeStart]);
  const maxValue = useMemo(() => {
    const values = displaySlots.map((slot) => (mode === 'offers' ? slot.averageOffers : slot.averageParticipants));
    return Math.max(1, ...values);
  }, [displaySlots, mode]);
  const bestSlot = useMemo(
    () => [...displaySlots].sort((a, b) => (b.averageOffers - a.averageOffers) || (b.averageParticipants - a.averageParticipants))[0],
    [displaySlots],
  );
  ...
  return (
    <div ref={chartRef} className="statistics-chart-card statistics-weekly-profile group/chart-card rounded-lg p-3 md:p-6" data-pdf-section onClick={onCardClick}>
      ...
      {activeSlot && tooltipPosition && typeof document !== 'undefined' && createPortal(
        <div
        ...
        >
          <span className="statistics-chart-tooltip-label block">{DAY_LABELS[activeSlot.weekday]} {slotLabel(activeSlot)}</span>
          <span className="statistics-chart-tooltip-value block">{activeSlot.averageOffers.toFixed(1)} Angebote parallel</span>
          <span className="statistics-chart-tooltip-value block">{activeSlot.averageParticipants.toFixed(1)} Besucher:innen je Angebot</span>
        </div>,
        document.body,
      )}
