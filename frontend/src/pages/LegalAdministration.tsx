    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-gray-600">
          Organisationsspezifische Texte importieren oder die aktuell verwendete Fassung
          herunterladen.
        </p>
      </div>
      ...
      <section className="system-data-panel overflow-hidden rounded-xl border shadow-sm">
        <div className="system-data-panel-header border-b px-5 py-4">
          ...
          <p className="system-data-panel-copy mt-1 text-sm">
            Wähle einen oder mehrere Texte aus. Nur die ausgewählten Dateien werden ersetzt und
        ...
        <div className="space-y-4 p-5">
            ...
            return (
              <div
                ...
                className={`flex flex-wrap items-center justify-between gap-3 rounded-xl border px-4 py-3 transition-colors ${
                  selectedFile
                    ? 'legal-import-document-selected border-violet-400 bg-violet-50 ring-1 ring-violet-200'
              ...
              >
                <span className="min-w-0">
                  ...
                  <span
                    className={`block text-xs ${selectedFile ? 'legal-import-document-selected-filename font-medium text-violet-800' : 'text-gray-500'}`}
                  >
                    {selectedFile?.name || hint}
                  </span>
                  {selectedFile ? (
                    <span
                      className="legal-import-document-selected-status mt-1 flex items-center gap-1 text-xs font-semibold text-violet-700"
                      role="status"
                    >
                      <AlertCircle className="h-3.5 w-3.5" aria-hidden="true" />
                      Datei ausgewählt – Import noch ausstehend
                    </span>
                  ) : null}
                  <span className="mt-1 block text-xs font-medium text-viridian">
                    Zuletzt aktualisiert: {formatUpdatedAt(current.data?.documents[key].updatedAt)}
                  </span>
                </span>
                <span className="flex shrink-0 items-center gap-2">
                  ...
                  <label
                    htmlFor={`legal-file-${key}`}
                    className={`cursor-pointer rounded-lg border bg-white px-3 py-2 text-sm font-medium hover:bg-gray-50 ${selectedFile ? 'legal-import-file-select-selected border-violet-300 text-violet-800' : 'border-gray-300 text-gray-700'}`}
                  >
                    {selectedFile ? 'Datei ändern' : 'Datei wählen'}
                  </label>
                </span>
                <input
                  id={`legal-file-${key}`}
                  ref={(element) => {
                    inputRefs.current[key] = element || undefined;
                  }}
                  type="file"
          ...
          {files.terms ? (
            <p className="system-data-banner system-data-banner-warning rounded-xl px-4 py-3 text-sm">
              Durch den Import der Nutzungsbedingungen müssen bestehende Nutzer:innen der neuen
              Version erneut zustimmen.
            </p>
          ) : null}
          <button
            type="button"
          ...
          >
            {importMutation.isPending
              ? 'Texte werden importiert …'
              : 'Ausgewählte Texte importieren'}
          </button>
        </div>
      </section>
    </div>
