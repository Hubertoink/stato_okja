...
import { BookOpen, CheckCircle2, ChevronRight, Filter, MessageCircle, Plus, Search, UserRound } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between gap-3 border-t border-gray-100 pt-3 text-xs text-gray-500">
        <span className="flex min-w-0 items-center gap-1.5"><span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-viridian/10 font-semibold text-viridian">{entry.createdByName.slice(0, 1).toUpperCase()}</span><span className="truncate">{entry.createdByName}</span></span>
...
export default function Logbook() {
  ...
  const { data, isLoading } = useLogbookEntries(filters);
  ...
  return (
    <div className="mx-auto max-w-6xl">
      ...
      ) : <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">{entries.map((entry) => <LogbookCard key={entry.id} entry={entry} />)}</div>}
    </div>
  );
}
