...
import { Archive, ArrowLeft, BarChart3, CheckCircle2, Copy, Download, ExternalLink, FileDown, Pencil, Play, Plus, Printer, QrCode, Trash2, Users } from 'lucide-react';
...
import { createSurveyTemplate, type SurveyQuestion, useCloseSurvey, useCreateSurveyRound, useDeleteSurveyResponse, useStartSurvey, useSurvey, useSurveyAnalytics, useSurveyResponses, useSurveyRounds, useSurveyTrend } from '@/lib/surveys';
...
export default function SurveyDetail() {
  ...
  return <div className="space-y-5"><PageHeader title={rounds[0]?.title || survey.title} description={`${survey.questions.length} Fragen · Runde ${survey.roundNumber || 1} · ${survey.responsesCount} Antworten`} actions={<div className="flex flex-wrap justify-end gap-2"><Button variant="secondary" onClick={() => navigate('/surveys')}><ArrowLeft className="h-4 w-4" /> Übersicht</Button><Button variant="secondary" onClick={exportTemplate}><Download className="h-4 w-4" /> Vorlage exportieren</Button><Button variant="secondary" onClick={createNextRound} disabled={createRound.isPending || survey.status === 'active'}><Plus className="h-4 w-4" /> Neue Umfragerunde</Button>{survey.status === 'draft' ? <Button onClick={() => start.mutate(survey.id, { onSuccess: () => showToast('Umfragerunde gestartet.'), onError: (error: any) => showToast(error?.response?.data?.message || 'Umfrage konnte nicht gestartet werden.', { type: 'error' }) })}><Play className="h-4 w-4" /> Starten</Button> : null}{survey.status === 'active' ? <Button variant="danger" onClick={() => setCloseConfirmOpen(true)}><CheckCircle2 className="h-4 w-4" /> Beenden</Button> : null}<Button variant="secondary" onClick={() => setEdit(true)} disabled={survey.status !== 'draft'}><Pencil className="h-4 w-4" /> Bearbeiten</Button></div>} />
