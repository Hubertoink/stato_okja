...
export default function PublicSurvey() {
  ...
  if (error && !survey) return <><main className="flex min-h-screen items-center justify-center bg-slate-50 p-5"><section className="max-w-md rounded-3xl bg-white p-8 text-center shadow-xl"><h1 className="text-2xl font-bold text-emerald-900">{t('public.unavailable')}</h1><p className="mt-3 text-slate-600">{error}</p></section></main>{legalModals}</>;
  if (!survey) return <main className="flex min-h-screen items-center justify-center bg-slate-50"><div className="rounded-2xl bg-white px-5 py-4 text-slate-600 shadow">{t('public.loading')}</div></main>;
